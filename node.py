...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for possible_split in possible_splits:
            y_left = y[:possible_split + 1]
            y_right = y[possible_split + 1:]

            left_positive = np.count_nonzero(y_left)
            left_negative = len(y_left) - left_positive
            right_positive = np.count_nonzero(y_right)
            right_negative = len(y_right) - right_positive

            left = left_positive + left_negative
            right = right_positive + right_negative

            gini_left = 1 - (left_positive / left) ** 2 - (left_negative / left) ** 2
            gini_right = 1 - (right_positive / right) ** 2 - (right_negative / right) ** 2
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        if feature_subset:
            feature_subset = np.random.choice(n_features, feature_subset, replace=False)
        else:
